package com.example.oztoticpacappturistica.naturales
...
class NaturalesCulturales : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_naturales_culturales, container, false)

        val sitios = obtenerNaturales()
    ...
    private fun obtenerNaturales(): List<Sitio> {
        val sitioRepository = SitioRepository(AppDatabase.getInstance(requireContext()).sitioDao())
        return runBlocking {
            sitioRepository.obtenerSitiosPorCategoria("Naturales-Culturales")
        }
    }
}